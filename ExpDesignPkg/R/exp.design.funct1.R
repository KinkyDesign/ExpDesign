...
function(dataset,predictionFeature,parameters){
    ...
    if(length(depend.variable)!=0){
        ...
        if(r2 >= parameters$r2.threshold){
            ...
            desD.int<- optFederov(form.int,new.scores[,1:2],parameters$nTrials,criterion=parameters$criterion,eval=TRUE,DFrac=1,nRepeats=100)
        ...
        desD.int<- optFederov(form.int,protc.pca$x[,1:4],parameters$nTrials,eval=TRUE,DFrac=1,nRepeats=100)#
        desD.eval<- eval.design(form.int,desD.int$design,confounding=TRUE)
